...
def erreur_relative_racine_carre():
    t = linspace(0, 50000, 50000)
    ecart = []
    points = []
    for element in t:
        a = sqrt(element)
        b = ma_racine_caree(element)
        c = ((b-a)/a)
        ecart.append(c)
...
def erreur_relative_ULP_etude_sqrt():
    t = linspace(1, 2**32, 5001)
    ecart_relatif = []
    bis = []
    for element in t:
        a = ma_racine_caree(element)
        b = sqrt(element)
        bis.append(abs((b-a)/spacing(element)))
    return nanmax(bis), nanmin(bis), nanmean(bis), nanstd(bis), nanvar(bis)


def mon_arcsin_cordic_sqrt(x):
    return cordit_arctan(x/ma_racine_caree(1-x**2))

def erreur_relative_arcsin_sqrt():
    t = linspace(-1, 1, 5000)
    ecart = []
    points = []
    for element in t:
        a = arcsin(element)
        b = mon_arcsin_cordic_sqrt(element)
        c = ((b-a)/a)
        ecart.append(c)
        points.append(element)
    plot(points,ecart,'r')
