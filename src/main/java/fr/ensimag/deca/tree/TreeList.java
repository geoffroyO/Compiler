...
public abstract class TreeList<TreeType extends Tree> extends Tree {
	 ...
	 * iterators, to make sure all callers iterate the same way (Main, IfThenElse,
	 * While, ...). If external iteration is needed, use getList().
	 */
	 ...
	 * 
	 * It is possible to use setLocation() on a list, but it is also OK not to set
	 * it.
	 */
	@Override
	...
	protected String prettyPrintNode() {
		return super.prettyPrintNode() + " [List with " + getList().size() + " elements]";
	}
