...
public abstract class Tree {
	 ...
	 *         {@link #prettyPrint()}.
	 */
	protected String printNodeLine(PrintStream s, String prefix, boolean last, boolean inlist) {
	 ...
	 *
	 * This displays the prefix (to show the tree hierarchy in ASCII-art), and the
	 * node name and information.
	 *
	 * @param s
	 * @param prefix
	 * @param last
	 ...
	 *         {@link #prettyPrint()}.
	 */
	String printNodeLine(PrintStream s, String prefix, boolean last, boolean inlist, String nodeName) {
		s.print(prefix);
		if (inlist) {
			s.print("[]>");
		} else if (last) {
	...
	/**
	 * Pretty-print tree (see {@link #prettyPrint()}), sending output to PrintStream
	 * s.
	 *
	 * @param s
	 */
	...
	protected final void prettyPrint(PrintStream s, String prefix, boolean last) {
		prettyPrint(s, prefix, last, false);
	}

	/**
	 * Pretty-print tree (see {@link #prettyPrint()}). This is an internal function
	 * that should usually not be called directly.
	 *
	 * @param s
	 *            Stream to send the output to
	 * @param prefix
	 *            Prefix (ASCII-art showing hierarchy) to print for this node.
	 * @param last
	 *            Whether the node being displayed is the last child of a tree.
	 * @param inlist
	 *            Whether the node is being displayed as part of a list.
	 */
	protected final void prettyPrint(PrintStream s, String prefix, boolean last, boolean inlist) {
	...
	/**
	 * Check that the current node has correctly been decorated, and throws an error
	 * if not.
	 *
	 * This is used only for defensive programming, each node type can add checks by
	 * overloading this method. Does nothing by default.
	 *
	 ...
	 *
	 * @return true. Raises an exception in case of error. The return value is meant
	 *         to allow assert(tree.checkAllLocations()), to enable the defensive
	 ...
	 *
	 * By default, this checks that getLocation() does not return null, but can be
	 * overridden for particular classes that do not require location information.
	 */
	protected void checkLocation() {
		if (getLocation() == null) {
			LOG.info(prettyPrint());
			throw new DecacInternalError("Tree " + getClass().getName() + " has no location set");
		}
	}
